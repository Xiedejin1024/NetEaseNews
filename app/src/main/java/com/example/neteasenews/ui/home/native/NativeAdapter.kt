...
class NativeAdapter(context: NativeFragment) : BaseAdapter() {

    private var datas: ArrayList<NativeDetail>
    private var mContext: NativeFragment

    init {
        mContext = context
        datas = ArrayList<NativeDetail>()
    }
    ...
    override fun getView(position: Int, converView: View?, viewGroup: ViewGroup?): View? {
        ...
        val nativeDetail = datas[position]
        Glide.with(NetEaseApplication.context).load(nativeDetail.imgsrc).into(holder.imageView)
        holder!!.tv_title!!.text = nativeDetail.title
        holder.tv_source!!.text = nativeDetail.source
        holder.tv_replyCount!!.text = nativeDetail.replyCount.toString() + "跟贴"
        return converView
    }

    fun upAdapterData(data: List<NativeDetail>) {
        datas.clear()
        datas.addAll(data)
